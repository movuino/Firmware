...
const unsigned long testIncrement = 1;//4096
...
void setup() {
 ...
 // digitalWrite(12, LOW);
  ...
  //create a file 
/*  SerialFlash.create("toto.txt", 4000);
  bool result=SerialFlash.createErasable("toto.txt", 4000);
  if(result) Serial.println("File created successfully !");
  else Serial.println("File creation failed  !!!!!");*/
}
...
bool test() {
  ...
  while (address < chipsize) { //    Serial.print(", data = ");
...
/* sig[0]=0;
 sig[1]=0;
 SerialFlash.write(0, sig, 1);
 delay(10);
 SerialFlash.read(0, buf, 2);
 Serial.print("buf0:");
    Serial.print(buf[0],HEX);
    Serial.print("buf1:");
    Serial.print(buf[1],HEX);
